...
### clustering TE copie ###
my $classi ;
if ( not defined ($classiFile)) { print STDERR "*** Could not find file containing the classification of the library (option -classi)  ***\n"  and die ; }
open (LIST, $classiFile) or die and print STDERR  "*** Could not open file containing the classification of the library : \"$classiFile\" ***\n" ;
while(<LIST>) {
	chomp $_ ;
	if ($_=~/^#/) {next ;}
	my @tab = split(" ", $_) ;
	unless (scalar(@tab) == 2)  { print STDERR ("*** Problems of format in \"$classiFile\" : wrong number of columns, need 2 columns ***\n") and die ; }
...
my $LTR ;
if (not defined ($annotLTR_File) ) { print STDERR ("*** Could not find file of position of LTR (option -LTR)***\n") and die ; }
open (LTR, $annotLTR_File) or die and print STDERR ("*** Can't open file of position of LTR \"$annotLTR_File\" ***\n")  ;
while (<LTR>) {
	chomp $_ ;
	if ($_=~/^#/) {next ;}
	my @tab = split("\t",$_) ;
	unless (scalar(@tab) == 5) { print STDERR ("*** Problems of format in \"$annotLTR_File\" : wrong number of columns, need 5 columns ***\n")  and die ; }
	unless ($tab[2] eq "LTR5" or $tab[2] eq "LTR3") { print STDERR ("*** Problems of format in \"$annotLTR_File\" : third column \"$tab[2]\" should be \"LTR 5'\" or \"LTR 3'\" ***\n") and die }
...
foreach my $file (@ARGV) {
	...
	if ($file=~/^(.*).xm$/) { @annotTE = &readXM ($file) ; }
	...
	EXIT:my $outputName = $PrefixFileName."_anno.".$outputFormat ;
...
sub _createEmbl {
	...
	foreach $_ (@{$final}) {
		...
		if ( $_->has_tag("parent") ) {
print "ID in final: " . ($_->each_tag_value('id'))[0] . "\n";
			my $parent = ($_->each_tag_value('parent'))[0] ;
print "Parent: " . $parent . "\n";
print Dumper($_) if ($parent eq "mp43");
print "\n";
...
print "Builded location: " . Dumper($location_obj) . "\n";
			} # END OF FOREACH $k
			#-------------------------------------------------------------------------------------
...
sub createParentFeature {
	...
	print "createParentFeature coord 1: " . Dumper(${$obj_feat1}->location()) . "\n";
	print "createParentFeature coord 2: " . Dumper(${$obj_feat2}->location()) . "\n";
	...
	if (${$obj_feat1}->{info}->{targetFamID} ne "exon" and ${$obj_feat2}->{info}->{targetFamID} ne "exon") {
		if (${$obj_feat1}->primary_tag eq "match_part" ) {
			foreach my $targStart (keys %{${$obj_feat1}->{range}}){
				print "Range feature 1: " . ${$obj_feat1}->{range}->{$targStart} . "\n";
				$feature->{range}->{$targStart} = ${$obj_feat1}->{range}->{$targStart};
			}
		} else {
			$feature->{range}->{${$obj_feat1}->{info}->{targetStart}} ="${$obj_feat1}->{info}->{targetStart}..${$obj_feat1}->{info}->{targetEnd}" ;
			print "Range feature 1 else: " . "${$obj_feat1}->{info}->{targetStart}..${$obj_feat1}->{info}->{targetEnd}" . "\n";
		}
		if (${$obj_feat2}->primary_tag eq "match_part" ) {
			foreach my $targStart (keys %{${$obj_feat2}->{range}}){
				print "Range feature 2: " . ${$obj_feat2}->{range}->{$targStart} . "\n";
				$feature->{range}->{$targStart} = ${$obj_feat2}->{range}->{$targStart};
			}
		} else {
			$feature->{range}->{${$obj_feat2}->{info}->{targetStart}} ="${$obj_feat2}->{info}->{targetStart}..${$obj_feat2}->{info}->{targetEnd}" ;
			print "Range feature 2 else: " . "${$obj_feat2}->{info}->{targetStart}..${$obj_feat2}->{info}->{targetEnd}" . "\n";
		}
	}
...
"LTRresolve" => wrong LTR prediction resolve by the program
