...
embl_files=$(cat results/chrom/${1}_embl_list)
...
bin/embl2gff.pl -RMclariTE -featurePrefix $2 -source clariTE -note -l 10 ${embl_files} > results/chrom/${1}_clariTE.gff

###### gff to gff3: new coordinates calculation with gawk commands "match" + "substr" (from chunks relative to chrom relative)
grep -v $'\t''region' results/chrom/${1}_clariTE.gff \
|awk -v FS='\t' -v OFS='\t' '/^chr/ {match($1, /:[0-9]+-/); $4=$4+substr($0, RSTART+1, RLENGTH-2); $5=$5+substr($0, RSTART+1, RLENGTH-2); print}' \
|gt gff3 -sort -tidy -retainids 1> results/chrom/${1}_clariTE_tmp.gff3  2> logs/chrom/${1}_clariTE_tmp_gt.log
...
grep -v -P '##sequence-region *'$1':[1-9]' results/chrom/${1}_clariTE_tmp.gff3 \
|awk -v LG=$1 -v end=$endchrom 'BEGIN{FS="\t";OFS="\t"} { if ($0~"##sequence-region") $0="##sequence-region\t"LG"\t1\t"end; print }' \
|sed -E 's/'$1':[0-9]*-[0-9]*/'$1'/' |sed -E 's/Compo:.* (Family)/\1/' | sed -E 's/Post:.* (Status)/\1/' > results/chrom/${1}_clariTE.gff3
