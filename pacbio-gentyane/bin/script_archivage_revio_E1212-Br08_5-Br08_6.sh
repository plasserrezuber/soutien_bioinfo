...
tar --exclude='*hifi_reads.unassigned.bam' --exclude='*.hifi_reads_demux.bc*.bam' --exclude='*.hifi_reads.default.bam' -czvhf  ${smrtcell_name}.${archive}.metadata.tar.gz fail_reads/ hifi_reads/ metadata/ pb_formats/ statistics/
...
for i in pacbio-data-provisoire/$mypath/hifi_reads/*.hifi_reads_demux.[a-d]*.bam; do ln -sr $i .; done
...
for i in *.hifi_reads_demux.[a-d]*.bam; do md5sum $i; done
echo -e "##############################################################################\n"
...
for i in pacbio-data-provisoire/$mypath/hifi_reads/*.hifi_reads_demux.[a-d]*.bam; do s3cmd -c ~/.s3cfg_gentyane signurl s3://$i  $(echo "`date +%s` + 3600 * 24 * 14" | bc); echo " "; done |  sed "s/http/https/g" 	
